...
- (UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width - kStatusBarHeight - kNavigationBarHeight, self.view.frame.size.height - kMinHeight) style:UITableViewStylePlain];
...
- (XMChatBar *)chatBar {
    if (!_chatBar) {
        _chatBar = [[XMChatBar alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - kMinHeight - kNavigationBarHeight - kStatusBarHeight, self.view.frame.size.width, kMinHeight)];
