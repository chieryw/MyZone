...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    addBarFrame.origin.y = self.screenHeight - self.bottomHeight - addBarFrame.size.height - kNavigationBarHeight - kStatusBarHeight;
...
- (void)faceViewSendFace:(NSString *)faceName{
    if ([faceName isEqualToString:@"[删除]"]) {
        ...
        self.inputText = @"";
        self.textView.text = @"";
        [self setFrame:CGRectMake(0, self.screenHeight - self.bottomHeight - kMinHeight - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, kMinHeight) animated:NO];
...
- (void)showViewWithType:(XMFunctionViewShowType)showType{
    ...
    switch (showType) {
        ...
        case XMFunctionViewShowVoice:
        {
            ...
            [self setFrame:CGRectMake(0, self.screenHeight - kMinHeight - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, kMinHeight) animated:NO];
        ...
        case XMFunctionViewShowFace:
            self.inputText = self.textView.text;
            [self setFrame:CGRectMake(0, self.screenHeight - kFunctionViewHeight - self.textView.frame.size.height - 10 - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, self.textView.frame.size.height + 10) animated:NO];
            [self.textView resignFirstResponder];
            [self textViewDidChange:self.textView];
            break;
...
- (void)showFaceView:(BOOL)show{
    if (show) {
        ...
        [UIView animateWithDuration:.3 animations:^{
            [self.faceView setFrame:CGRectMake(0, self.screenHeight - kFunctionViewHeight - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, kFunctionViewHeight)];
        } completion:nil];
    }else{
        [UIView animateWithDuration:.3 animations:^{
            [self.faceView setFrame:CGRectMake(0, self.screenHeight - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, kFunctionViewHeight)];
...
- (void)showMoreView:(BOOL)show{
    if (show) {
        ...
        [UIView animateWithDuration:.3 animations:^{
            [self.moreView setFrame:CGRectMake(0, self.screenHeight - kFunctionViewHeight - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, kFunctionViewHeight)];
        } completion:nil];
    }else{
        [UIView animateWithDuration:.3 animations:^{
            [self.moreView setFrame:CGRectMake(0, self.screenHeight - kStatusBarHeight - kNavigationBarHeight, self.frame.size.width, kFunctionViewHeight)];
        } completion:^(BOOL finished) {
...
- (void)sendTextMessage:(NSString *)text{
    ...
    self.inputText = @"";
    self.textView.text = @"";
    [self setFrame:CGRectMake(0, self.screenHeight - self.bottomHeight - kMinHeight - kNavigationBarHeight - kStatusBarHeight, self.frame.size.width, kMinHeight) animated:NO];
...
- (XMChatFaceView *)faceView{
    if (!_faceView) {
        _faceView = [[XMChatFaceView alloc] initWithFrame:CGRectMake(0, self.screenHeight - kStatusBarHeight - kNavigationBarHeight , self.frame.size.width, kFunctionViewHeight)];
...
- (XMChatMoreView *)moreView{
    if (!_moreView) {
        _moreView = [[XMChatMoreView alloc] initWithFrame:CGRectMake(0, self.screenHeight - kStatusBarHeight - kNavigationBarHeight , self.frame.size.width, kFunctionViewHeight)];
