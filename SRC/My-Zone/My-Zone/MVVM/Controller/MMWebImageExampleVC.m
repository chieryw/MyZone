...
CGFloat const progressLayerHeight = 4;
...
- (void)initSelf {
    ...
    [self.label addGestureRecognizer:g];
}
...
- (CAShapeLayer *)progressLayer {
    if (!_progressLayer) {
        ...
        _progressLayer.size = CGSizeMake(self.webImageView.width, progressLayerHeight);
        UIBezierPath *path = [UIBezierPath bezierPath];
        [path moveToPoint:CGPointMake(0, _progressLayer.height / 2)];
        [path addLineToPoint:CGPointMake(self.webImageView.width, _progressLayer.height / 2)];
        _progressLayer.lineWidth = progressLayerHeight;
...
@implementation MMWebImageExampleVC {
    NSArray *_imageLinks;
}
